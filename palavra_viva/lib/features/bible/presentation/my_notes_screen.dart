...
class _MyNotesScreenState extends ConsumerState<MyNotesScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final progressState =
        ref.watch(bibleProgressProvider).value ?? const BibleProgressState();
      ...
      body: TabBarView(
        ...
        children: [
          _MarksList(
            marks: notes,
            books: books,
            type: _ListType.notes,
            isDark: isDark,
          ),
          _MarksList(
            marks: highlights,
            books: books,
            type: _ListType.highlights,
            isDark: isDark,
          ),
        ],
      ),
...
class _MarksList extends StatelessWidget {
  ...
  BibleBook? _findBook(String abbrev) {
    try {
      return books.firstWhere((b) => b.abbrev == abbrev);
    } catch (_) {
      return null;
    }
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Toque em um versículo para começar',
          ...
          onTap: () {
            if (book != null) {
              Navigator.of(context, rootNavigator: true).push(
                MaterialPageRoute(
                  builder: (_) =>
                      ReadingScreen(book: book, chapterIndex: mark.chapter - 1),
                ),
              );
            }
          },
                  ...
                  children: [
                    if (mark.highlightColor != null)
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: HighlightColors.toColor(mark.highlightColor),
